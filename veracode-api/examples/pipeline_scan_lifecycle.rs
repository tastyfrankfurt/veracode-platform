use veracode_platform::{
    ...
    pipeline::{CreateScanRequest, ScanConfig, DevStage, PipelineError, Finding}
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sample_file_path = possible_paths.iter()
    ...
    println!("   Binary Hash: {} (length: {} chars)", binary_hash, binary_hash.len());
    ...
    let (upload_uri, expected_segments) = if let (Some(uri), Some(segments)) = (&scan_result.upload_uri, scan_result.expected_segments) {
    ...
    let file_name = sample_file_path.split('/').next_back().unwrap_or("binary.tar.gz");
    
    let upload_successful = match pipeline_api.upload_binary_segments(
        &upload_uri,
        expected_segments,
        &binary_data,
        file_name
    ...
    let start_scan_result = if let Some(ref start_uri) = scan_result.start_uri {
        ...
        pipeline_api.start_scan_with_uri(start_uri, Some(scan_config)).await
    } else {
    ...
    match pipeline_api.get_scan(scan_id).await {
        Ok(scan) => {
            ...
            println!("   Binary: {}", scan.binary_name);
            println!("   Status: {}", scan.scan_status);
            println!("   Upload Progress: {}/{} segments", scan.binary_segments_uploaded, scan.binary_segments_expected);
            println!("   Created: {}", scan.created);
    ...
    match pipeline_api.get_results(scan_id).await {
        Ok(results) => {
            ...
            match serde_json::to_string_pretty(&results) {
                Ok(json_data) => {
                    match std::fs::write(&results_filename, json_data) {
                        Ok(_) => {
            ...
            if !results.findings.is_empty() {
                ...
                for (i, finding) in results.findings.iter().take(3).enumerate() {
                    println!("   {}. {} (CWE-{})", i + 1, finding.issue_type, finding.cwe_id);
                    println!("      File: {} (line {})", finding.files.source_file.file, finding.files.source_file.line);
                    println!("      Severity: {} ({})", 
                             get_severity_name(finding.severity), finding.severity);
                    println!("      Title: {}", finding.title);
                    if !finding.files.source_file.function_name.as_deref().unwrap_or("").is_empty() {
                        println!("      Function: {}", finding.files.source_file.function_name.as_deref().unwrap_or(""));
                    }
        ...
        Err(PipelineError::FindingsNotReady) => {
            ...
            match pipeline_api.get_scan(scan_id).await {
                Ok(scan) => {
                    ...
                    if scan.scan_status.is_successful() {
                        println!("\n🔍 Scan shows {}, attempting to get findings separately...", scan.scan_status);
                        match pipeline_api.get_findings(scan_id).await {
                            ...
                            Err(PipelineError::FindingsNotReady) => {
                                println!("⏳ Findings still not ready - may need more time to process");
                        ...
                        println!("   You can run this example again once the scan reaches SUCCESS status");
                    }
                }
                Err(e) => {
...
/// Save findings to CSV file for easy analysis
fn save_findings_to_csv(findings: &[Finding], filename: &str) -> Result<(), Box<dyn std::error::Error>> {
    use std::fs::File;
    use std::io::Write;
    ...
    // Write CSV header
    writeln!(file, "File,Line,Function,Issue Type,Severity,Severity Name,Title,CWE ID,Issue ID,Grade,Issue Type ID")?;
    ...
    for finding in findings {
        writeln!(
            ...
            finding.files.source_file.function_name.as_deref().unwrap_or("").replace("\"", "\"\""),

use veracode_platform::{
    ...
    scan::{UploadFileRequest, BeginPreScanRequest, BeginScanRequest, ScanError},
    CreateApplicationRequest, CreateSandboxRequest
};
use veracode_platform::app::{CreateApplicationProfile, BusinessCriticality};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let app_id = match client.create_application(app_request).await {
        Ok(app) => {
            ...
            println!("   Application Name: {}", app.profile.as_ref().map(|p| &p.name).unwrap_or(&app_name));
            app.guid
        }
        Err(e) => {
    ...
    let sample_file_path = match sample_file_path {
        ...
        None => {
            ...
            println!("   1. Package your application code into a supported format (.war, .jar, .zip, etc.)");
    ...
    let upload_request = UploadFileRequest {
        ...
        save_as: Some(format!("{}-{}.tar.gz", scan_name.replace(" ", "_"), timestamp)),
    ...
    match scan_api.begin_prescan(prescan_request).await {
        Ok(build_id) => {
            ...
            match scan_api.get_prescan_results(&app_id, Some(&sandbox_id), Some(&build_id)).await {
                Ok(prescan_results) => {
                    ...
                    for (i, module) in prescan_results.modules.iter().enumerate() {
                        println!("   Module {}: {} ({})", i + 1, module.name, module.module_type);
                        println!("      Selected: {}, Fatal: {}", module.selected, module.is_fatal);
                    }
                    
                    // Display messages
                    for (i, message) in prescan_results.messages.iter().enumerate() {
                        println!("   Message {}: [{}] {}", i + 1, message.severity, message.text);
                    }
                }
                Err(e) => {
            ...
            match scan_api.begin_scan(scan_request).await {
                Ok(scan_build_id) => {
                    ...
                    match scan_api.get_build_info(&app_id, Some(&scan_build_id), Some(&sandbox_id)).await {
                        Ok(scan_info) => {
    ...
    match scan_api.upload_and_scan_sandbox(&app_id, &sandbox_id, sample_file_path).await {
        Ok(scan_build_id) => {
    ...
    match client.delete_application(&app_id).await {
        ...
        Err(e) => {
            ...
            println!("💡 The application may need to be manually deleted from the Veracode platform");
        }
    }
...
async fn demonstrate_api_methods(
    ...
    match scan_api.get_build_info(app_id, None, Some(sandbox_id)).await {
        Ok(scan_info) => {
    ...
    match scan_api.get_prescan_results(app_id, Some(sandbox_id), None).await {
        Ok(prescan_results) => {
            println!("✅ Pre-scan results retrieved:");
            println!("   Build ID: {}", prescan_results.build_id);
            println!("   Status: {}", prescan_results.status);

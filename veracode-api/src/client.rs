...
impl VeracodeClient {
    ...
    fn generate_hmac_signature(&self, method: &str, url: &str, timestamp: u64, nonce: &str) -> Result<String, VeracodeError> {
        ...
        let data = format!("id={}&host={}&url={}&method={}", 
                          self.config.api_id, 
                          host, 
                          path_and_query, 
                          method);
        ...
        let key_bytes = hex::decode(&self.config.api_key)
            .map_err(|_| VeracodeError::Authentication("Invalid API key format - must be hex string".to_string()))?;
    ...
    pub fn generate_auth_header(&self, method: &str, url: &str) -> Result<String, VeracodeError> {
        ...
        Ok(format!("VERACODE-HMAC-SHA-256 id={},ts={},nonce={},sig={}",
                   self.config.api_id, timestamp, nonce, signature))
    }
    ...
    pub async fn get(&self, endpoint: &str, query_params: Option<&[(String, String)]>) -> Result<reqwest::Response, VeracodeError> {
    ...
    ///
    /// A `Result` containing the HTTP response.
    pub async fn post<T: Serialize>(&self, endpoint: &str, body: Option<&T>) -> Result<reqwest::Response, VeracodeError> {
    ...
    ///
    /// A `Result` containing the HTTP response.
    pub async fn put<T: Serialize>(&self, endpoint: &str, body: Option<&T>) -> Result<reqwest::Response, VeracodeError> {
        let url = format!("{}{}", self.config.base_url, endpoint);
    ...
    pub async fn handle_response(response: reqwest::Response) -> Result<reqwest::Response, VeracodeError> {
        if !response.status().is_success() {
            let status = response.status();
            let error_text = response.text().await?;
            return Err(VeracodeError::InvalidResponse(
    ...
    pub async fn get_with_query(&self, endpoint: &str, query_params: Option<Vec<(String, String)>>) -> Result<reqwest::Response, VeracodeError> {
    ...
    ///
    /// A `Result` containing the HTTP response, pre-processed for success/failure.
    pub async fn post_with_response<T: Serialize>(&self, endpoint: &str, body: Option<&T>) -> Result<reqwest::Response, VeracodeError> {
    ...
    ///
    /// A `Result` containing the HTTP response, pre-processed for success/failure.
    pub async fn put_with_response<T: Serialize>(&self, endpoint: &str, body: Option<&T>) -> Result<reqwest::Response, VeracodeError> {
    ...
    ///
    /// A `Result` containing the HTTP response, pre-processed for success/failure.
    pub async fn delete_with_response(&self, endpoint: &str) -> Result<reqwest::Response, VeracodeError> {
        ...
        loop {
            ...
            if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(&response_text) {
                ...
                if let Some(embedded) = json_value.get("_embedded") {
                    if let Some(items_array) = embedded.as_object().and_then(|obj| obj.values().next()) {
    ...
    pub async fn get_with_params(&self, endpoint: &str, params: &[(&str, &str)]) -> Result<reqwest::Response, VeracodeError> {
        let url = format!("{}{}", self.config.base_url, endpoint);
        let mut request_url = Url::parse(&url).map_err(|e| VeracodeError::InvalidConfig(e.to_string()))?;
    ...
    ///
    /// A `Result` containing the response or an error.
    pub async fn post_form(&self, endpoint: &str, params: &[(&str, &str)]) -> Result<reqwest::Response, VeracodeError> {
        let url = format!("{}{}", self.config.base_url, endpoint);
        ...
        F: Fn(u64, u64, f64) + Send + Sync,
    {
        ...
        if file_size > MAX_FILE_SIZE {
            return Err(VeracodeError::InvalidConfig(
                format!("File size ({} bytes) exceeds maximum limit of {} bytes", file_size, MAX_FILE_SIZE)
            ));
        }
        ...
        let request = self.client
            ...
            .header("Authorization", auth_header)
            .header("User-Agent", "Veracode Rust Client")
            .header("Content-Type", content_type.unwrap_or("binary/octet-stream"))

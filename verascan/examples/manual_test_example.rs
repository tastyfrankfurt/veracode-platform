...
use verascan::findings::{AggregatedFindings, FindingWithSource, ScanSource, ScanMetadata, AggregationStats, CweStatistic, create_finding_hash};
use veracode_platform::pipeline::{Finding, SourceFile, FindingFiles, FindingsSummary, ScanStatus};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Severity: {} ({})", aggregated.findings[0].finding.severity, get_severity_name(aggregated.findings[0].finding.severity));
    println!("   File: {}", aggregated.findings[0].finding.files.source_file.file);
    println!("   Line: {}", aggregated.findings[0].finding.files.source_file.line);
...
fn check_environment_variables() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token = env::var("PRIVATE_TOKEN")
        ...
        .map_err(|_| "Missing required environment variable: PRIVATE_TOKEN, CI_TOKEN, or GITLAB_TOKEN")?;

...
fn execute_policy_download_with_runtime(veracode_config: VeracodeConfig, policy_name: &str, args: &Args) -> Result<(), i32> {
    ...
    rt.block_on(async {
        download_policy_by_name(veracode_config, policy_name, args).await
    })
}

async fn download_policy_by_name(veracode_config: VeracodeConfig, policy_name: &str, args: &Args) -> Result<(), i32> {
    ...
    if args.debug {
        println!("✅ Found policy: {} (GUID: {})", target_policy.name, target_policy.guid);
    }
    
    // Get the full policy details
    let full_policy = policy_api.get_policy(&target_policy.guid).await
        .map_err(|e| {
    ...
    let json_content = serde_json::to_string_pretty(&full_policy)
        .map_err(|e| {

...
impl std::fmt::Display for ValidationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ValidationError::UnsupportedFileType(msg) => write!(f, "Unsupported file type: {}", msg),
...
impl FileValidator {
    ...
    pub fn validate_file(&self, file_path: &Path, debug: bool) -> Result<SupportedFileType, ValidationError> {
        // Read the first 512 bytes to check file signatures
        let mut file = File::open(file_path).map_err(|e| {
            ValidationError::IoError(format!("Failed to open file '{}': {}", file_path.display(), e))
        })?;

        let mut buffer = vec![0; 512];
        let bytes_read = file.read(&mut buffer).map_err(|e| {
            ValidationError::IoError(format!("Failed to read file '{}': {}", file_path.display(), e))
        })?;
        ...
        if bytes_read == 0 {
            return Err(ValidationError::InvalidFileHeader("File is empty".to_string()));
        }
        ...
        if debug {
            println!("🔍 DEBUG: First 16 bytes: {:02x?}", &buffer[..std::cmp::min(16, buffer.len())]);
        }
        ...
        if debug {
            ...
            println!("🔍 DEBUG: Infer detected type: {:?}", file_type.as_ref().map(|t| t.mime_type()));
        }
            ...
            Some(kind) => {
                match kind.mime_type() {
                        ...
                        match file_extension.as_deref() {
                            Some("jar") => {
                                ...
                                if self.is_jar_file(&buffer, debug) {
                                    ...
                                    if debug {
                                        println!("🔍 DEBUG: JAR structure not found, treating as ZIP");
                                    }
                            ...
                            Some("war") => {
                                ...
                                if self.is_war_file(&buffer, debug) {
                                    ...
                                    if debug {
                                        println!("🔍 DEBUG: WAR structure not found, treating as ZIP");
                                    }
                        ...
                        if file_extension.as_deref() == Some("gz") && 
                           file_path.to_string_lossy().ends_with(".tar.gz") {
                            ...
                            Err(ValidationError::UnsupportedFileType(
                                format!("GZIP file without .tar.gz extension: {}", file_path.display())
                            ))
                        }
                    },
                    "application/x-bzip2" => {
                        if file_extension.as_deref() == Some("bz2") && 
                           file_path.to_string_lossy().ends_with(".tar.bz2") {
                            ...
                            Err(ValidationError::UnsupportedFileType(
                                format!("BZIP2 file without .tar.bz2 extension: {}", file_path.display())
                            ))
                        }
                    },
                    ...
                    _ => Err(ValidationError::UnsupportedFileType(
                        format!("File type '{}' not supported for Veracode scanning", kind.mime_type())
                    )),
                }
            },
            None => {
                ...
                match file_extension.as_deref() {
                    ...
                    _ => Err(ValidationError::UnsupportedFileType(
                        format!("Unable to determine file type for: {}", file_path.display())
                    )),
                }
            }
        }
    ...
    fn has_zip_signature(&self, buffer: &[u8]) -> bool {
        ...
        matches!(buffer[0..4], [0x50, 0x4B, 0x03, 0x04] | 
                              [0x50, 0x4B, 0x05, 0x06] | 
                              [0x50, 0x4B, 0x01, 0x02])
    }
    ...
    pub fn is_suitable_for_static_analysis(&self, file_type: &SupportedFileType) -> bool {
        match file_type {
            SupportedFileType::Jar | 
            SupportedFileType::War | 
            SupportedFileType::Zip => true,
            SupportedFileType::Tar | 
            SupportedFileType::TarGz | 
            SupportedFileType::TarBz2 => true, // May contain source code or compiled artifacts
            SupportedFileType::SevenZip |
            SupportedFileType::Rar => false, // Less common for Java applications
        }
    }
}
...
mod tests {
    ...
    fn test_file_type_descriptions() {
        ...
        assert!(validator.get_file_type_description(&SupportedFileType::Jar).contains("Java"));
        assert!(validator.get_file_type_description(&SupportedFileType::War).contains("Web"));
    }

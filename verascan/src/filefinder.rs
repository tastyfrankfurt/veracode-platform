...
impl FileFinder {
    ...
    pub fn parse_config(directory: &str, patterns_str: &str, recursive: bool, validate_files: bool, debug: bool) -> Result<SearchConfig, SearchError> {
        // Validate directory
        ...
        let patterns: Result<Vec<Pattern>, _> = patterns_str
            ...
            .map(|pattern| {
                Pattern::new(pattern).map_err(|e| {
                    SearchError::InvalidPattern(format!("'{}': {}", pattern, e))
                })
            })
            .collect();
        ...
        if patterns.is_empty() {
            return Err(SearchError::InvalidPattern("No valid patterns provided".to_string()));
        }
    ...
    pub fn search(&self, config: &SearchConfig) -> Result<Vec<PathBuf>, SearchError> {
        ...
        if config.recursive {
            self.search_recursive(&config.directory, &config.patterns, &mut matched_files, config)?;
        } else {
            self.search_directory(&config.directory, &config.patterns, &mut matched_files, config)?;
        }
    ...
    fn search_directory(&self, dir_path: &Path, patterns: &[Pattern], matched_files: &mut Vec<PathBuf>, config: &SearchConfig) -> Result<(), SearchError> {
        let entries = fs::read_dir(dir_path).map_err(|e| {
            SearchError::IoError(format!("Failed to read directory '{}': {}", dir_path.display(), e))
        })?;
        ...
        for entry in entries {
            ...
            let file_type = match entry.file_type() {
                Ok(ft) => ft,
                Err(e) => {
                    eprintln!("Warning: Could not determine file type for {}: {}", entry.path().display(), e);
                    continue;
                }
            };
                    ...
                    if pattern.matches(&file_name_str) {
                        ...
                        if config.validate_files {
                            match self.validator.validate_file(&file_path, config.debug) {
                                Ok(file_type) => {
                                    let suitable = self.validator.is_suitable_for_static_analysis(&file_type);
                                    if config.debug {
                                        println!("✅ Valid file: {}", file_path.display());
                                        println!("   Type: {} ({})", file_type, self.validator.get_file_type_description(&file_type));
                                        println!("   Static Analysis: {}", if suitable { "✅ Suitable" } else { "❌ Not recommended" });
                                    } else {
                                        println!("✅ Valid file: {}", file_path.display());
                                    }
    ...
    fn search_recursive(&self, dir_path: &Path, patterns: &[Pattern], matched_files: &mut Vec<PathBuf>, config: &SearchConfig) -> Result<(), SearchError> {
...
mod tests {
    ...
    fn test_parse_config_valid() {
        ...
        let config = FileFinder::parse_config(&temp_path, "*.txt,*.rs", false, false, false).unwrap();
    ...
    fn test_parse_config_invalid_directory() {
        let result = FileFinder::parse_config("/nonexistent/directory", "*.txt", false, false, false);

...
impl GitLabIssuesClient {
    ...
    pub async fn validate_gitlab_connection(debug: bool) -> Result<(), GitLabError> {
        ...
        if status.is_success() {
            ...
            let project_path = project_info["path_with_namespace"].as_str().unwrap_or("unknown/project");
            ...
            if issues_response.status().is_success() {
                ...
                println!("   Issue creation: ⚠️  May be restricted (status: {})", issues_response.status());
            }
    ...
    pub fn from_env(debug: bool) -> Result<Self, GitLabError> {
        ...
        if env::var("VERASCAN_DISABLE_CERT_VALIDATION").is_ok() {
            if debug {
                println!("⚠️  WARNING: Certificate validation disabled via VERASCAN_DISABLE_CERT_VALIDATION");
    ...
    pub async fn create_issues_from_findings(&mut self, aggregated: &AggregatedFindings) -> Result<Vec<GitLabIssueResponse>, GitLabError> {
        if self.debug {
            println!("📝 Creating GitLab issues from {} findings", aggregated.findings.len());
        }
        ...
        for (index, finding_with_source) in aggregated.findings.iter().enumerate() {
            ...
            match self.issue_already_exists(&issue_payload.title).await {
                Ok(true) => {
                    if self.debug {
                        println!("⏭️  Skipping duplicate issue {}/{}: {}", 
                            index + 1, 
                ...
                Err(e) => {
                    eprintln!("⚠️  Warning: Failed to check for duplicates for {}: {}. Creating issue anyway.", finding.title, e);
    ...
    async fn create_issue(&self, payload: &GitLabIssuePayload) -> Result<GitLabIssueResponse, GitLabError> {
        let url = format!("{}{}/issues", self.config.gitlab_url, self.config.project_id);
    ...
    async fn issue_already_exists(&self, title: &str) -> Result<bool, GitLabError> {
        ...
        let url = format!("{}{}/issues?search={}&in=title&state=opened&labels=security::veracode", 
    ...
    fn create_issue_payload(&self, finding_with_source: &crate::findings::FindingWithSource) -> Result<GitLabIssuePayload, GitLabError> {
        ...
        if self.debug {
            ...
            println!("   Raw file path from Veracode: '{}'", finding.files.source_file.file);
            println!("   Issue type: '{}'", finding.issue_type);
            println!("   Severity: {} ({})", finding.severity, self.get_severity_name(finding.severity));
        ...
        let function_or_issue = finding.files.source_file.function_name
            .as_ref()
        ...
        let description = self.create_issue_description_with_resolved_path(finding_with_source, &resolved_file_path)?;
        
        // Create labels based on severity and issue type
        let mut labels = vec![
            format!("security::severity::{}", severity_name.to_lowercase().replace(" ", "-")),
        ...
        let function_name = finding.files.source_file.function_name
            .as_ref()
            .filter(|f| !f.is_empty() && *f != "UNKNOWN")
            .map(|s| s.as_str())
            .unwrap_or("");
    ...
    fn resolve_file_path(&self, file_path: &str) -> String {
        ...
        if self.debug {
            println!("   Path is_absolute: {}, is_relative: {}", 
    ...
    fn find_file_by_relative_path(&self, project_dir: &Path, relative_path: &str) -> Option<String> {
        if self.debug {
            ...
            if candidate_path.exists() && candidate_path.is_file() {
                ...
                if let Some(candidate_str) = candidate_path.to_str() {
                    ...
                    if normalized_candidate.ends_with(&normalized_relative) {
                        ...
                        if match_start == 0 || normalized_candidate.chars().nth(match_start - 1) == Some('/') {
                            ...
                            println!("     ⚠️  Path ends with target but not at boundary: {}", normalized_candidate);
                        }
                    }
                } else if self.debug {
                    println!("     ⚠️  Could not convert path to string: {}", candidate_path.display());
                }
            }
        }
    ...
    fn search_for_file(&self, current_dir: &Path, filename: &str, project_root: &Path) -> Option<String> {
    ...
    fn create_issue_description_with_resolved_path(&self, finding_with_source: &crate::findings::FindingWithSource, resolved_file_path: &str) -> Result<String, GitLabError> {
        let finding = &finding_with_source.finding;
        ...
        description.push_str(&format!("![{}](https://img.shields.io/badge/Severity-{}-{})\n\n", 
            severity_name, 
        ...
        description.push_str(&format!("| **CWE** | {} |\n", 
            if finding.cwe_id.is_empty() || finding.cwe_id == "0" { 
                ...
                format!("[CWE-{}](https://cwe.mitre.org/data/definitions/{}.html)", finding.cwe_id, finding.cwe_id)
            }));
        ...
        if let Some(ref flaw_details_link) = finding.flaw_details_link {
            if !flaw_details_link.is_empty() {
                description.push_str(&format!("| **Flaw Details** | [View in Veracode]({}) |\n", flaw_details_link));
                if self.debug {
        ...
        if let Some(project_web_url) = self.get_project_web_url() {
            ...
            description.push_str(&format!("🔗 **[View code at line {}]({})**\n\n", finding.files.source_file.line, file_url));
        }
        ...
        if let Some(ref pipeline_id) = self.config.pipeline_id {
            let pipeline_url = format!("{}{}/pipelines/{}", 
        ...
        if let Some(ref flaw_details_link) = finding.flaw_details_link {
            if !flaw_details_link.is_empty() {
                links_section.push_str(&format!("- [Detailed Vulnerability Information (Veracode)]({})\n", flaw_details_link));
                has_links = true;
                if self.debug {
                    println!("   Added flaw details link to Related Links section: {}", flaw_details_link);
                }
            } else {
                if self.debug {
        ...
        description.push_str(&format!("This {} vulnerability requires attention. ", finding.issue_type));
        description.push_str("Please review the identified code and apply appropriate security measures:\n\n");
        description.push_str("1. **Review** the vulnerable code in the identified file and function\n");
        description.push_str("2. **Research** the specific vulnerability type and remediation techniques\n");
        description.push_str("3. **Apply** security fixes following secure coding practices\n");
        description.push_str("4. **Test** thoroughly to ensure the fix doesn't break functionality\n");
        ...
        // Footer
        description.push_str("---\n\n");
        description.push_str("*This issue was automatically created by Verascan security scanning.*");
...
impl GitLabConfig {
    ...
    pub fn from_env() -> Result<Self, GitLabError> {
        ...
        let api_token = env::var("PRIVATE_TOKEN")
            ...
            .map_err(|_| GitLabError::MissingEnvVar("PRIVATE_TOKEN, CI_TOKEN, or GITLAB_TOKEN".to_string()))?;
...
mod tests {
    ...
    fn test_issue_title_url_encoding() {
        ...
        assert_eq!(encoded, "%5BHigh%5D%20SQL%20Injection%20in%20UserController.java");
    ...
    fn test_labels_format() {
        ...
        assert_eq!(labels_string, "security::veracode,security::sast,security::severity::high,priority::high");
    ...
    fn test_path_matching_precision() {
        ...
        assert!(!ends_with_match, "SimpleVulnerableApp.java should not match VulnerableApp.java");
        ...
        assert!(ends_with_match2, "SimpleVulnerableApp.java should match itself");
        ...
        assert_eq!(boundary_char, Some('/'), "Should have path separator at boundary");
    }

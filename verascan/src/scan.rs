...
pub fn validate_baseline_file_early(baseline_path: &str, args: &Args) -> Result<(), i32> {
    ...
    let metadata = fs::metadata(path).map_err(|e| {
        eprintln!(
            "❌ Cannot read baseline file metadata '{}': {}",
            baseline_path, e
        );
        1
    })?;
    ...
    if file_size > MAX_BASELINE_SIZE {
        eprintln!(
            "❌ Baseline file is too large ({} bytes): '{}'. Maximum allowed: {} bytes",
    ...
    let baseline: BaselineFile = serde_json::from_value(json_value.clone()).map_err(|e| {
        eprintln!(
            "❌ Baseline file '{}' does not match expected baseline format: {}",
            baseline_path, e
        );
    ...
    if baseline.metadata.version.is_empty() {
        eprintln!(
            "❌ Baseline file '{}' has empty version in metadata",
            baseline_path
        );
        return Err(1);
    }
    ...
    if !baseline.findings.is_empty() {
        ...
        if first_finding.finding_id.is_empty() || first_finding.cwe_id.is_empty() {
            eprintln!(
                "❌ Baseline file '{}' contains findings with empty required fields (finding_id, cwe_id)",
                baseline_path
            );
            return Err(1);
        }
...
fn parse_dev_stage(stage_str: &str) -> veracode_platform::pipeline::DevStage {
    ...
    match stage_str.to_lowercase().as_str() {
        ...
        _ => {
            ...
            eprintln!(
                "⚠️  Warning: Invalid development stage '{}', defaulting to Development",
                stage_str
            );
    ...
    } = &args.command
    {
        ...
        if let Commands::Pipeline { export_format, .. } = &args.command {
            ...
            match export_format_lower.as_str() {
                ...
                _ => {
                    eprintln!(
                        "❌ Unsupported export format: {}. Use 'json', 'csv', 'gitlab', or 'all'",
                        export_format_lower
                    );
                }
            }
...
fn ensure_extension(path: &str, extension: &str) -> Result<PathBuf, String> {
    ...
    if path_buf.exists() && path_buf.is_dir() {
        return Err(format!(
            "Export path '{}' is a directory. Please specify a file path.",
            path
        ));
    }
    ...
    if let Commands::Pipeline { project_dir, .. } = &args.command {
        match GitLabIssuesClient::from_env(args.debug) {
            Ok(mut client) => {
                ...
                if args.debug {
                    println!(
                        "📁 Using project directory for file path resolution: {}",
                        project_dir
                    );
                }
...
async fn execute_assessment_scan_async(
    ...
    } = &args.command
    {
        if args.debug {
            println!(
                "🔍 Looking up or creating application profile: {}",
                app_profile_name
            );
        }
        ...
        let app_id = match submitter
            ...
            .create_application_if_not_exists(
                ...
                Some(format!(
                    "Application created by Verascan for assessment scanning"
                )),
                team_names,
            )
            .await
        {
            ...
            Err(e) => {
                eprintln!(
                    "❌ Failed to lookup or create application '{}': {}",
                    app_profile_name, e
                );
                return Err(1);
            }

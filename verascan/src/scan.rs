...
pub fn validate_baseline_file_early(baseline_path: &str, args: &Args) -> Result<(), i32> {
    ...
    if file_size > MAX_BASELINE_SIZE {
        eprintln!("❌ Baseline file is too large ({} bytes): '{}'. Maximum allowed: {} bytes", 
                 file_size, baseline_path, MAX_BASELINE_SIZE);
        return Err(1);
    }
    ...
    let json_value: serde_json::Value = serde_json::from_str(&content).map_err(|e| {
        eprintln!("❌ Baseline file '{}' is not valid JSON: {}", baseline_path, e);
        1
    })?;
    
    // Try to parse as BaselineFile structure
    let baseline: BaselineFile = serde_json::from_value(json_value.clone()).map_err(|e| {
        eprintln!("❌ Baseline file '{}' does not match expected baseline format: {}", baseline_path, e);
    ...
    if baseline.metadata.version.is_empty() {
        eprintln!("❌ Baseline file '{}' has empty version in metadata", baseline_path);
        return Err(1);
    }
    
    if baseline.metadata.finding_count != baseline.findings.len() as u32 {
        eprintln!("⚠️  Warning: Baseline file '{}' metadata.finding_count ({}) does not match actual findings count ({})", 
                 baseline_path, baseline.metadata.finding_count, baseline.findings.len());
    }
    ...
    if !baseline.findings.is_empty() {
        ...
        if first_finding.finding_id.is_empty() || first_finding.cwe_id.is_empty() {
            eprintln!("❌ Baseline file '{}' contains findings with empty required fields (finding_id, cwe_id)", baseline_path);
            return Err(1);
        }
    }
    ...
    if args.debug {
        ...
        println!("      - Source project: {}", baseline.metadata.source_scan.project_name);
        println!("      - Created at: {}", baseline.metadata.created_at);
    } else {
        println!("✅ Baseline file validated successfully ({} findings)", baseline.findings.len());
    }
...
fn parse_dev_stage(stage_str: &str) -> veracode_platform::pipeline::DevStage {
    ...
    match stage_str.to_lowercase().as_str() {
        ...
        _ => {
            ...
            eprintln!("⚠️  Warning: Invalid development stage '{}', defaulting to Development", stage_str);
...
fn resolve_git_project_url(debug: bool) -> Option<String> {
    ...
    let git_config_path = match find_git_config_file() {
        ...
        None => {
            if debug {
                println!("   ⚠️  No .git/config file found in current directory or parent directories");
            }
            return None;
        }
    };
...
fn execute_scan_with_runtime(submitter: PipelineSubmitter, matched_files: &[PathBuf], args: &Args) -> Result<(), i32> {
...
async fn execute_single_scan(submitter: &PipelineSubmitter, matched_files: &[PathBuf], args: &Args) -> Result<(), i32> {
...
async fn execute_concurrent_scans(submitter: &PipelineSubmitter, matched_files: &[PathBuf], args: &Args) -> Result<(), i32> {
    match submitter.submit_files_concurrent_and_wait(matched_files).await {
        Ok(results_vec) => {
...
fn display_concurrent_results(results_vec: &[veracode_platform::pipeline::ScanResults], submitter: &PipelineSubmitter) {
    println!("\n📊 All Pipeline Scans Completed!");
    println!("   Total scans: {}", results_vec.len());
    
    for (index, results) in results_vec.iter().enumerate() {
        println!("\n--- Scan {} of {} ({}) ---", index + 1, results_vec.len(), results.scan.binary_name);
        submitter.display_results_summary(results);
    }
    
    let total_findings: u32 = results_vec.iter().map(|r| r.summary.total).sum();
    let total_high_critical: u32 = results_vec.iter().map(|r| r.summary.very_high + r.summary.high).sum();
...
async fn handle_findings_export(results_vec: &[veracode_platform::pipeline::ScanResults], matched_files: &[PathBuf], args: &Args) {
    if let Commands::Pipeline { export_findings, .. } = &args.command {
        ...
        let aggregated = if let Some(min_severity) = severity_filter {
            if args.debug {
                println!("🔽 Applying severity filter: {} and above", 
...
fn ensure_extension(path: &str, extension: &str) -> Result<PathBuf, String> {
    ...
    if path_buf.exists() && path_buf.is_dir() {
        return Err(format!("Export path '{}' is a directory. Please specify a file path.", path));
    }
    
    // Check if parent directory exists (if there is one)
    if let Some(parent) = path_buf.parent() {
        if !parent.as_os_str().is_empty() && !parent.exists() {
            return Err(format!("Parent directory '{}' does not exist. Please create it first.", parent.display()));
        }
    }
    ...
    if final_path.exists() {
        println!("⚠️  Warning: File '{}' already exists and will be overwritten.", final_path.display());
    }
...
[branch "main"]
    ...
    fn test_extract_ssh_url_parts() {
        ...
        assert_eq!(result, Some(("github.com".to_string(), "user/repo.git".to_string())));
...
fn handle_baseline_operations(aggregated: &AggregatedFindings, args: &Args) -> AggregatedFindings {
    ...
    if let Commands::Pipeline { baseline_file: Some(baseline_path), .. } = &args.command {
        ...
        match execute_baseline_compare(aggregated, baseline_path, None, args.debug) {
            Ok(comparison) => {
                if args.debug {
                    println!("Total flaws found: {}, New flaws found: {} as compared to baseline ({} baseline flaws ignored)", 
                ...
                if baseline_failed {
                    if policy_enabled {
                        ...
                        if args.debug {
                            println!("\n🔍 {} new findings detected vs baseline - applying policy criteria", comparison.summary.new_count);
                        }
                    } else {
                        // Standalone baseline comparison
                        println!("\n❌ Baseline comparison FAILED - {} new findings detected", comparison.summary.new_count);
                        println!("   Baseline created: {}", comparison.metadata.baseline_info.created_at);
            ...
            Err(_) => {
                eprintln!("❌ The \"baseline_file\" does not contain JSON formatting. Correct the file and try again.");
...
async fn export_pass_fail_violations(
    ...
    if let Some(assessment) = policy_assessment {
        if !assessment.violations.is_empty() {
            ...
            criteria_description.push(format!("Policy '{}' violations", assessment.metadata.policy_info.name));
    ...
    if !violating_findings.is_empty() {
        export_pass_fail_filtered_findings(&violating_findings, &criteria_description, output_path, args.debug);
        // Exit with non-zero code when pass-fail criteria are violated
        println!("\n❌ Pass-fail criteria FAILED - {} violations detected", violating_findings.len());
        std::process::exit(1);
    } else {
...
fn evaluate_fail_on_severity(findings: &[FindingWithSource], severity_list: &str) -> Vec<FindingWithSource> {
    // Parse severity list
    let target_severities: Vec<u32> = severity_list.split(',')
        .map(|s| parse_severity_name_to_level(s.trim()))
...
fn evaluate_fail_on_cwe(findings: &[FindingWithSource], cwe_list: &str) -> Vec<FindingWithSource> {
    // Parse CWE list
    let target_cwes: Vec<String> = cwe_list.split(',')
        .map(|s| {
            let cwe = s.trim();
...
fn calculate_cwe_breakdown(findings: &[FindingWithSource]) -> serde_json::Value {
    ...
    for finding in findings {
        *breakdown.entry(finding.finding.cwe_id.clone()).or_insert(0u32) += 1;
    }

...
pub fn execute_file_search(args: &Args) -> Result<Vec<PathBuf>, i32> {
    let (filepath, filefilter, recursive, validate) = match &args.command {
        Commands::Pipeline { filepath, filefilter, recursive, validate, .. } => {
            (filepath, filefilter, *recursive, *validate)
        },
        Commands::Assessment { filepath, filefilter, recursive, validate, .. } => {
            (filepath, filefilter, *recursive, *validate)
        },
        Commands::Policy { .. } => {
...
fn display_search_results(matched_files: &[PathBuf], args: &Args, filefilter: &str, recursive: bool, validate: bool) -> Result<(), i32> {
    let search_type = if recursive { "recursively" } else { "in directory" };
    
    if matched_files.is_empty() {
        match &args.command {
            Commands::Pipeline { .. } => {
                eprintln!("❌ No files found for pipeline scan upload");
                eprintln!("💡 Ensure files matching the pattern '{}' exist in the specified directory", filefilter);
                return Err(1);
            },
            Commands::Assessment { .. } => {
            ...
            Commands::Policy { .. } => {
                println!("No files found {} matching the patterns: {}", search_type, filefilter);
                return Ok(());
            },
        }
    }
    ...
    if validate {
        ...
        println!("Found {} file(s) {} matching patterns '{}':", matched_files.len(), search_type, filefilter);
